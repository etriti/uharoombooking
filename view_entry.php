...
echo "<h3 class='view_entry_title'" . (($keep_private && $is_private_field['entry.name']) ? " class=\"private\"" : "") . ">\n";
...
echo "<table class='table table-striped table-bordered' id=\"entry\">\n";
...
<div id="view_entry_nav">
  ...
  if (!$room_disabled)
  {
    ...
    if (!$series)
    {
      echo "<a class='btn btn-default edit' href=\"edit_entry.php?id=$id&amp;returl=$link_returl\">". get_vocab("editentry") ."</a>";
    }
    ...
    if ((!empty($repeat_id) || $series) && $repeats_allowed)
    {
      echo "<a class='btn btn-default edit' href=\"edit_entry.php?id=$id&amp;edit_type=series&amp;day=$day&amp;month=$month&amp;year=$year&amp;returl=$link_returl\">".get_vocab("editseries")."</a>";
    }
    echo "</div>\n";
    ...
    if (!$series)
    {
      echo "<a class='btn btn-default delete' href=\"del_entry.php?id=$id&amp;series=0&amp;returl=$link_returl\" onclick=\"return confirm('".get_vocab("confirmdel")."');\">".get_vocab("deleteentry")."</a>";
    }
    if (!empty($repeat_id) && !$series && $repeats_allowed)
    {
      echo " - ";
    ...
    if ((!empty($repeat_id) || $series) && $repeats_allowed)
    {
      echo "<a class='btn btn-default delete' href=\"del_entry.php?id=$id&amp;series=1&amp;day=$day&amp;month=$month&amp;year=$year&amp;returl=$link_returl\" onClick=\"return confirm('".get_vocab("confirmdel")."');\">".get_vocab("deleteseries")."</a>";
    }
    echo "</div>\n";
  }
  ...
  if (!$series)
  {
    echo "<a class='btn btn-default copy' href=\"edit_entry.php?id=$id&amp;copy=1&amp;returl=$link_returl\">". get_vocab("copyentry") ."</a>";
  }
  if (!empty($repeat_id) && !$series && $repeats_allowed)
  {
    echo " - ";
  ...
  if ((!empty($repeat_id) || $series) && $repeats_allowed)
  {
    echo "<a class='btn btn-default copy' href=\"edit_entry.php?id=$id&amp;edit_type=series&amp;day=$day&amp;month=$month&amp;year=$year&amp;copy=1&amp;returl=$link_returl\">".get_vocab("copyseries")."</a>";
  }
  echo "</div>\n";
  ...
  if (!$keep_private && !$enable_periods)
  {
    ...
    if (!$series)
    {
      echo "<a class='btn btn-default export' href=\"view_entry.php?action=export&amp;id=$id&amp;returl=$link_returl\">". get_vocab("exportentry") ."</a>";
    }
    if (!empty($repeat_id)  && !$series)
    {
      echo " - ";
    ...
    if (!empty($repeat_id) || $series)
    {
      echo "<a class='btn btn-default export' href=\"view_entry.php?action=export&amp;id=$repeat_id&amp;series=1&amp;day=$day&amp;month=$month&amp;year=$year&amp;returl=$link_returl\">".get_vocab("exportseries")."</a>";
    }
    echo "</div>\n";
  }
  ?>
  <div id="returl">
    ...
    {
    ?>
    <a class="btn btn-default return" href="<?php echo htmlspecialchars($HTTP_REFERER) ?>"><?php echo get_vocab("returnprev") ?></a>
    <?php
    }
    ?>
  </div>
